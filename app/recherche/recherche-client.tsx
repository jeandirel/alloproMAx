...
import {Search,Map,LocateFixed,SlidersHorizontal,ChevronDown} from 'lucide-react'
...
// to cityId, else fall back to normalized-text matching (legacy demo Pro
// records only ever carry free-text zone/zones). Falls through to the next
// tier whenever the current one yields no candidates.
...
// catalogue explorer), but only if it actually yields candidates. Falls
...
interface RechercheClientProps{
 initialQuery:string;initialCat:string;initialZone?:string;initialCategoryId?:string;initialSubcategoryId?:string
 initialProvinceId?:string;initialCityId?:string;initialNeighborhoodId?:string
 realPros:Professional[];realCategories:{id:string;name:string}[]
}
export function RechercheClient({initialQuery,initialCat,initialZone='',initialCategoryId='',initialSubcategoryId='',initialProvinceId='',initialCityId='',initialNeighborhoodId='',realPros,realCategories}:RechercheClientProps){
 const ws=useOptionalWorkspace();const state=ws?.state;const [query,setQuery]=useState(initialQuery);const [cat,setCat]=useState(initialCat);const [online,setOnline]=useState(false);const [verified,setVerified]=useState(false);const [rating,setRating]=useState(0);const [maxPrice,setMaxPrice]=useState('');const [maxDistance,setMaxDistance]=useState('');const [sort,setSort]=useState('note');const [map,setMap]=useState(false);const [filters,setFilters]=useState(false);const [moreFilters,setMoreFilters]=useState(false);const [zone,setZone]=useState(initialZone);const [zoneLoc,setZoneLoc]=useState<LocationPickerValue|null>(initialProvinceId?{provinceId:initialProvinceId,cityId:initialCityId||null,neighborhoodId:initialNeighborhoodId||null,displayName:initialZone}:null);const [serviceLoc,setServiceLoc]=useState<CatalogueExplorerValue|null>(null);const [position,setPosition]=useState<Point|null>(null)
 ...
 // Fiches réelles (base de données) toujours incluses ; le « Workspace » de
 // démonstration (compte connecté) s'y ajoute sans jamais s'y substituer.
 const data=useMemo(()=>{if(!state)return realPros;const merged=[...state.pros.filter(p=>!('suspended'in p&&p.suspended)),...realPros];return merged.filter((p,i,arr)=>arr.findIndex(q=>q.id===p.id)===i)},[state,realPros])
 const cats=state?.categories.filter(c=>c.active).map(c=>c.name)||realCategories.map(c=>c.name);const origin=position||state?.profile.location||zonePoints[state?.profile.quartier||'Libreville Centre']||zonePoints['Libreville Centre']
 const filtered=useMemo(()=>{let list=data.filter(p=>!cats.length||cats.includes(p.categorie));if(query.trim())list=list.filter(p=>normalize(`${p.name} ${p.metier} ${p.categorie} ${p.services.map(s=>s.nom).join(' ')}`).includes(normalize(query)));if(cat)list=list.filter(p=>normalize(p.categorie).includes(normalize(cat)));list=filterByService(list,serviceLoc);list=filterByZone(list,zoneLoc,zoneName);if(online)list=list.filter(p=>p.enLigne);if(verified)list=list.filter(p=>p.verifie);if(rating)list=list.filter(p=>p.note>=rating);if(maxPrice)list=list.filter(p=>p.tarifMin<=Number(maxPrice));if(maxDistance)list=list.filter(p=>{const pt=zonePoints[p.zone];return pt?distanceKm(origin,pt)<=Number(maxDistance):false});return list.sort((a,b)=>{if(sort==='prix')return a.tarifMin-b.tarifMin;if(sort==='prix-desc')return b.tarifMin-a.tarifMin;if(sort==='distance'){const da=zonePoints[a.zone]?distanceKm(origin,zonePoints[a.zone]):Infinity;const db=zonePoints[b.zone]?distanceKm(origin,zonePoints[b.zone]):Infinity;return da-db}return b.note-a.note})},[data,cats,query,cat,serviceLoc,zoneLoc,zoneName,online,verified,rating,maxPrice,maxDistance,origin,sort])

 const filterCount=Number(online)+Number(verified)+Number(rating>0)+Number(Boolean(maxPrice))+Number(Boolean(maxDistance))
 ...
 return <main className="ap-client-page">
   <div className="mb-7 rounded-[2rem] bg-primary/5 p-5 sm:p-8"><PageHeading title="Le bon professionnel, près de vous" subtitle="Recherchez un service, choisissez votre zone, et comparez les professionnels disponibles."/>
   ...
   <div className="grid items-start gap-6 lg:grid-cols-[240px_minmax(0,1fr)] xl:grid-cols-[260px_minmax(0,1fr)]">
     <aside id="search-filters" className={`${filters?'block':'hidden'} rounded-3xl border border-border/40 bg-white p-5 shadow-sm lg:block`}>
       ...
       <div className="grid gap-5">
         ...
         <button type="button" className="flex min-h-11 items-center justify-between gap-2 text-sm font-semibold text-emerald-dark" onClick={()=>setMoreFilters(!moreFilters)} aria-expanded={moreFilters}>Plus de filtres<ChevronDown size={16} className={moreFilters?'rotate-180 transition-transform':'transition-transform'}/></button>
         {moreFilters&&<div className="grid gap-5 border-t border-border/60 pt-5">
           ...
           <p className="text-xs leading-relaxed text-muted-foreground">Distances à vol d’oiseau, disponibles uniquement pour les zones déjà couvertes par notre localisation.</p>
         </div>}
     ...
     <section className="min-w-0" aria-label="Résultats de recherche">
       ...
       {map&&<div className="mb-5 overflow-hidden rounded-3xl"><ServiceMap markers={filtered.filter(p=>zonePoints[p.zone]).map(p=>({id:p.id,name:p.name,point:zonePoints[p.zone],href:`/professionnel/${p.id}`}))} position={position}/></div>}
       {data.length===0?<div className="rounded-3xl border border-border/50 bg-white p-8 text-center"><p className="text-sm text-muted-foreground">Aucun professionnel disponible dans cette zone pour le moment.</p><Link href="/signup" className="ap-button mt-5 inline-flex">Devenir l&apos;un des premiers professionnels</Link></div>:filtered.length?<div className="grid gap-5 sm:grid-cols-2">{filtered.map(p=><div key={p.id} className="flex min-w-0 flex-col"><div className="flex-1"><ProfessionalCard pro={p}/></div>{zonePoints[p.zone]&&<p className="mt-2 px-2 text-xs leading-relaxed text-muted-foreground">À environ {distanceKm(origin,zonePoints[p.zone]).toFixed(1)} km</p>}</div>)}</div>:<div><Empty text="Aucun professionnel ne correspond à ces critères. Essayez une autre catégorie ou élargissez la distance."/><button className="ap-secondary mt-4" onClick={reset}>Voir tous les professionnels</button></div>}
     </section>
   </div>
 </main>
}
