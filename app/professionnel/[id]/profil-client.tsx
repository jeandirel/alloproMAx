...
export function ProfilClient({ pro: original }: { pro: Professional }) {
  ...
  const hasRating = (pro.ratingCount ?? pro.avis.length) > 0
  const stats = [
    hasRating && { value: String(pro.note), label: 'Note sur 5', icon: Star },
    pro.missions > 0 && { value: String(pro.missions), label: 'Missions', icon: Users },
    pro.tauxReponse != null && { value: `${pro.tauxReponse}%`, label: 'Taux de réponse', icon: MessageSquare },
    pro.delaiMoyen && { value: pro.delaiMoyen, label: 'Délai moyen', icon: Clock },
  ].filter(Boolean) as { value: string; label: string; icon: typeof Star }[]
  ...
  return <main className="ap-client-page !pb-32 lg:!pb-12">
    ...
    <section className="overflow-hidden rounded-[2rem] border border-border/40 bg-white shadow-sm">
      <div className="ap-home-hero flex min-h-28 items-start justify-between gap-3 bg-primary px-5 py-5 text-white sm:px-8">
        <span className="rounded-full bg-white/15 px-3 py-1.5 text-xs font-semibold">{pro.categorie}{pro.zone ? ` · ${pro.zone}` : ''}</span>
      ...
      <div className="px-5 pb-6 sm:px-8 sm:pb-8">
        <div className="relative -mt-10 mb-4 aspect-square w-24 overflow-hidden rounded-3xl border-4 border-white bg-muted sm:w-28"><Image src={pro.photo} alt={`Photo de ${pro.name}`} fill sizes="112px" className="object-cover" priority/></div>
        ...
        {stats.length > 0 && <div className="mt-6 grid grid-cols-2 gap-3 border-t border-border/60 pt-6 sm:grid-cols-4">{stats.map(s=><div key={s.label} className="rounded-2xl bg-background p-4"><s.icon size={18} className="mb-2 text-primary"/><p className="font-display text-xl font-bold">{s.value}</p><p className="mt-1 text-xs text-muted-foreground">{s.label}</p></div>)}</div>}
      </div>
    </section>
    <div className="mt-6 grid items-start gap-6 lg:grid-cols-[minmax(0,1fr)_320px] xl:grid-cols-[minmax(0,1fr)_360px]">
      <div className="min-w-0 space-y-6">
        ...
        {(pro.galerie?.length??0)>0&&<section className="ap-profile-section"><h2>Un aperçu du savoir-faire</h2><div className="grid grid-cols-2 gap-3 sm:grid-cols-3">{pro.galerie.map((img,i)=><div key={i} className="relative aspect-square overflow-hidden rounded-2xl bg-muted"><Image src={img} alt={`Photo ${i+1} du travail de ${pro.name}`} fill className="object-cover" sizes="(max-width: 640px) 42vw, (max-width: 1024px) 28vw, 210px"/></div>)}</div></section>}
        <section className="ap-profile-section"><div className="mb-5 flex flex-wrap items-center justify-between gap-3"><h2 className="!mb-0">Avis clients</h2>{hasRating&&<span className="inline-flex items-center gap-1.5 text-sm"><Star size={16} className="fill-gold text-gold"/><strong>{pro.note}/5</strong></span>}</div>{(pro.avis??[]).length===0&&reviews.length===0?<p className="text-sm text-muted-foreground">Aucun avis pour le moment.</p>:<div className="divide-y divide-border/60">{(pro.avis??[]).map((a,i)=><article key={i} className="py-5 first:pt-0"><div className="mb-3 flex flex-wrap items-center gap-3"><div className="relative aspect-square w-10 shrink-0 overflow-hidden rounded-full bg-muted"><Image src={a.photo} alt={a.nom} fill sizes="40px" className="object-cover"/></div><div className="min-w-0 flex-1"><p className="text-sm font-semibold">{a.nom}</p><p className="mt-0.5 text-xs text-muted-foreground">{a.quartier} · {a.date}</p></div><StarRating rating={a.note}/></div><p className="text-sm leading-7 text-muted-foreground">{a.commentaire}</p></article>)}{reviews.map(m=><article key={m.id} className="py-5"><strong className="text-sm">{ws?.state?.profile.name}</strong><p className="my-2 text-xs text-muted-foreground">Qualité {m.review!.quality}/5 · Ponctualité {m.review!.punctuality}/5 · Communication {m.review!.communication}/5</p><p className="text-sm leading-7">{m.review!.comment}</p></article>)}</div>}</section>
      ...
      <aside className="space-y-5 lg:sticky lg:top-24">
        <div className="ap-profile-section"><p className="text-sm text-muted-foreground">Prestations à partir de</p><p className="mt-2 font-display text-3xl font-bold tracking-tight text-primary">{formatFCFA(pro.tarifMin)}</p><p className="mb-5 mt-3 text-xs leading-relaxed text-muted-foreground">Hors frais de plateforme. Consultez le détail avant de confirmer votre réservation.</p>
          <div className="ap-profile-actions"><div className="mx-auto flex max-w-6xl gap-2 lg:flex-wrap"><Link href={`/reserver/${pro.id}`} className="flex min-h-12 min-w-0 flex-1 items-center justify-center gap-2 rounded-xl bg-gold px-4 py-3 text-sm font-bold text-night transition-colors hover:bg-gold-dark lg:basis-full">Réserver <ArrowRight size={17}/></Link><Link aria-label="Envoyer un message" href={`/messages?thread=PRO-${pro.id}`} className="ap-secondary !px-4 lg:flex-1"><MessageSquare size={18}/><span className="hidden lg:inline">Message</span></Link>{phone?<a aria-label="Appeler le professionnel" href={`tel:${phone.replace(/[^+0-9]/g,'')}`} className="ap-secondary !px-4"><Phone size={18}/></a>:<button aria-label="Numéro non disponible" onClick={()=>toast.info('Ce professionnel n’a pas encore renseigné de numéro. Utilisez la messagerie pour le contacter.')} className="ap-secondary !px-4"><Phone size={18}/></button>}</div></div>
          <div className="mt-5 flex items-start gap-2 border-t border-border/60 pt-5 text-xs leading-relaxed text-muted-foreground"><CreditCard size={17} className="shrink-0 text-primary"/>Le tarif est à convenir directement avec le professionnel avant la prestation.</div>
        </div>
