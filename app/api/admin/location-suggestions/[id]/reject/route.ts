...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const suggestion = await prisma.locationSuggestion.findUnique({ where: { id }, select: { status: true } })
    ...
    const updated = await prisma.locationSuggestion.update({
      where: { id },
    ...
    console.error('Rejet suggestion localisation', e)
