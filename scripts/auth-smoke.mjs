...
// Reuse this authenticated session for runtime business-rule tests without logging credentials.
export async function sessionFetch(path, options = {}) {
  const res = await fetch(BASE + path, { ...options, headers: { ...options.headers, cookie: cookies() }, redirect: options.redirect || 'manual' });
  save(res);
  return res;
}
export { BASE };
