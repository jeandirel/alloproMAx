...
export function WorkspaceProvider({children}:{children:React.ReactNode}){
 ...
 useEffect(()=>{const change=()=>{setOnline(navigator.onLine);if(navigator.onLine)void reload()};const visible=()=>{if(!document.hidden)void reload()};window.addEventListener('online',change);window.addEventListener('offline',change);document.addEventListener('visibilitychange',visible);const timer=setInterval(()=>{if(!document.hidden&&navigator.onLine)void reload()},30000);change();return()=>{clearInterval(timer);window.removeEventListener('online',change);window.removeEventListener('offline',change);document.removeEventListener('visibilitychange',visible)}},[reload])
