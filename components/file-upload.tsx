...
export function FileUpload({label,value,onChange,imagesOnly=false}:{label:string;value:FileRef[];onChange:(v:FileRef[])=>void;imagesOnly?:boolean}){
 ...
 for(const f of Array.from(files)){if(f.size>10*1024*1024)throw new Error('Chaque fichier doit peser moins de 10 Mo.');if(imagesOnly&&!['image/jpeg','image/png','image/webp'].includes(f.type))throw new Error('Choisissez une image JPEG, PNG ou WebP.')
 ...
 return <div className="space-y-2"><span className="ap-label">{label}</span><label className="border border-dashed border-emerald-300 bg-emerald-50/50 rounded-xl p-4 flex gap-3 items-center cursor-pointer text-sm text-emerald-900">{busy?<Loader2 className="animate-spin"/>:<Upload size={20}/>}<span>{busy?'Téléversement en cours…':'Ajouter des fichiers'}<small className="block text-muted-foreground">{imagesOnly?'JPEG, PNG, WebP':'JPEG, PNG, WebP, PDF'} · 10 Mo maximum</small></span><input aria-label={label} disabled={busy} type="file" multiple accept={imagesOnly?'image/jpeg,image/png,image/webp':'image/jpeg,image/png,image/webp,application/pdf'} className="sr-only" onChange={e=>{void upload(e.target.files);e.target.value=''}}/></label>{value.map(f=><div key={f.id} className="flex gap-2 items-center text-sm"><FileCheck size={16} className="text-emerald-dark"/><a className="truncate underline" href={`/api/files?id=${encodeURIComponent(f.id)}`} download>{f.name}</a><button type="button" aria-label={`Retirer ${f.name}`} onClick={()=>onChange(value.filter(v=>v.id!==f.id))} className="p-2 ml-auto"><X size={16}/></button></div>)}</div>
}
