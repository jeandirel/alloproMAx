...
export function ProfessionalCard({ pro }: { pro: Professional }) {
  return <Link href={`/professionnel/${pro.id}`} className="group flex h-full flex-col rounded-3xl border border-border/50 bg-white p-5 shadow-sm transition-[box-shadow,border-color] hover:border-primary/30 hover:shadow-lg sm:p-6">
    ...
    <div className="flex items-center gap-3">
      <div className="relative aspect-square w-16 shrink-0 overflow-hidden rounded-2xl bg-muted"><Image src={pro.photo} alt={`Portrait illustratif de ${pro.name}`} fill className="object-cover" sizes="64px"/></div>
    ...
    <div className="mt-4 flex flex-wrap items-center gap-x-4 gap-y-2 text-xs text-muted-foreground">
      <span className="flex items-center gap-1"><Star className="h-3.5 w-3.5 fill-gold text-gold"/><strong className="text-foreground">{pro.note}</strong> · {pro.avis.length} avis</span>
      <span className="flex items-center gap-1"><MapPin size={13} className="shrink-0"/>{pro.zone}</span>
      <span className="flex items-center gap-1"><Clock size={13}/>{pro.delaiMoyen}</span>
    </div>
